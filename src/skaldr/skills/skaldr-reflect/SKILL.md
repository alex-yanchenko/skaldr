...
existing under a name you didn't reach for — proposing a fix for something that already ships wastes the
maintainer's time and erodes trust in the rest of your list. If it exists, that's a *docs/discoverability*
pain point instead (you looked and didn't find it), which is still worth reporting — just framed honestly.
